...
namespace Lean.Touch
{
    ...
    {
        [Header("Clamp")] [Tooltip("Should the plane be clamped on the x axis?")]
        public bool ClampX;
        ...
        [Header("Snap")] [Tooltip("The distance between each position snap on the x axis")]
        public float SnapX;
        ...
        public bool TryRaycast(Ray ray, ref Vector3 hit, float offset = 0.0f)
        {
            ...
            if (Raycast(point, normal, ray, ref distance))
            {
        ...
        public Vector3 GetClosest(Ray ray)
        {
            ...
            if (Raycast(point, normal, ray, ref distance)) return GetClosest(ray.GetPoint(distance));
