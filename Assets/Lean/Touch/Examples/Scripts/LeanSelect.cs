...
namespace Lean.Touch
{
    ...
    public class LeanSelect_Inspector : LeanInspector<LeanSelect>
    {
    }
}
#endif
    ...
    public class LeanSelect : MonoBehaviour
    {
            ...
            switch (selectUsing)
            {
                case SelectType.Raycast3D:
                {
                    ...
                    if (camera != null)
                    {
                        ...
                        if (Physics.Raycast(ray, out hit, float.PositiveInfinity, LayerMask)) component = hit.collider;
                    }
                    else
                    {
                        Debug.LogError(
            ...
            if (selectable != null && selectable.isActiveAndEnabled)
            {
                if (selectable.HideWithFinger)
                    for (var i = LeanSelectable.Instances.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (instance.HideWithFinger && instance.IsSelected) return;
                    }
        ...
        protected virtual void OnEnable()
        {
            if (Instances.Count > 0)
                Debug.LogWarning(
                    "Your scene already contains a LeanSelect component, using more than once at once may cause selection issues",
                    Instances[0]);
