...
namespace Lean.Common.Examples
{
    ...
    public class LeanCircuit_Inspector : LeanInspector<LeanCircuit>
    {
        ...
        {
            if (Target.Paths != null) currentPath = EditorGUILayout.IntSlider(currentPath, 0, Target.Paths.Count - 1);
                ...
                if (path.Points != null)
                {
                    ...
                    Handles.BeginGUI();
                    {
                        for (var i = 0; i < path.Points.Count; i++)
                        {
                            ...
                            var rect = new Rect(0.0f, 0.0f, 50.0f, 20.0f);
                        ...
                        for (var i = 1; i < path.Points.Count; i++)
                        {
                            ...
                            if (GUI.Button(
                                new Rect(scrPoint.x - 5.0f, Screen.height - scrPoint.y - 45.0f, 20.0f, 20.0f), "+"))
                            {
                                path.Points.Insert(i, midPoint);
                                dirty = true;
                            }
                        }
                    }
...
{
	/// <summary>
	///     This component generates a basic circuit mesh based on the specified paths, with circles at the end of each
	///     path, unless they intersect another.
	/// </summary>
	[ExecuteInEditMode]
    ...
    public class LeanCircuit : MonoBehaviour
    {
        ...
        private static readonly List<Vector2> coords = new List<Vector2>();
        ...
        [NonSerialized] private bool cachedMeshFilterSet;
        ...
        private void AddNode(Vector3 point)
        {
            for (var i = nodes.Count - 1; i >= 0; i--)
            {
                ...
                if (node.Increment(point)) return;
            }
